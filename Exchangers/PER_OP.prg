...
***
 ...
 DEFINE WINDOW beg FROM 5, 18 TO 10, 48 NOCLOSE TITLE ' Дата перелўку Обм.пунктўв ' NOMINIMIZE COLOR SCHEME 10
 ...
 a2 = SUBSTR(DTOC(p_reestr), 1, 2)+SUBSTR(DTOC(p_reestr), 4, 2)+SUBSTR(DTOC(p_reestr), 7, 2)+'КС'
 SELECT tmp
 ...
 WAIT WINDOW TIMEOUT 2 'Данў по перелўку ОП в файлах '+fal1
 DEACTIVATE WINDOW beg
 DEACTIVATE WINDOW glob
 CLOSE ALL
