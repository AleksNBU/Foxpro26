...
***
 ...
 DEFINE WINDOW beg FROM 5, 16 TO 10, 62 NOCLOSE TITLE ' Дати реїстрацў∙ Обм.пунктўв ' NOMINIMIZE COLOR SCHEME 10
 ...
 p_reestr = DATE()
 k_reestr = DATE()
 @ 1, 00 SAY 'Поч.дата звўтностў за'
 @ 2, 06 GET p_reestr SIZE 1, 10
 @ 1, 22 SAY 'Кўн.дата звўтностў за'
 @ 2, 25 GET k_reestr SIZE 1, 10
 READ
 ...
 USE dbfs\Rab0
 APPEND FROM (p_ath_d)
 COPY TO Dbfs\Rab1 FOR (dat_reestr>=p_reestr .AND. dat_reestr<=k_reestr)
 USE
 SELECT 0
 ...
 IF k_bo=0
    ...
    WAIT WINDOW TIMEOUT 2 'Даних на дати '+DTOC(p_reestr)+', '+DTOC(k_reestr)+' Нема ! '
    USE
    RETURN
 ENDIF
 USE
 ...
 SELECT rab
 USE
 a1 = SUBSTR(DTOC(p_reestr), 1, 2)+SUBSTR(DTOC(k_reestr), 1, 2)+SUBSTR(DTOC(p_reestr), 7, 2)+'OP'
 a2 = SUBSTR(DTOC(p_reestr), 1, 2)+SUBSTR(DTOC(k_reestr), 1, 2)+SUBSTR(DTOC(p_reestr), 7, 2)+'КС'
 SELECT tmp
 ...
 COUNT TO sm_0_1_2
 GOTO TOP
 REPORT FORMAT PER_OP_Z.FRX TO FILE (fal1) NOCONSOLE
 SET FILTER TO typ_ugod=0
 WAIT WINDOW TIMEOUT 2 'Данў по перелўку ОП в файлў '+fal1
 DEACTIVATE WINDOW beg
 DEACTIVATE WINDOW glob
 CLOSE ALL
